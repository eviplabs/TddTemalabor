    ...
    class AmountDiscountCalculator
    {
        ...
        public void ApplyDiscount(Dictionary<char, (int Actual, int ToCountBy)> ProductCount, List<Product> Products)
        {
            if(Discounts.Count > 0) 
            {
                ...
                {
                    if (Discounts.ContainsKey(key) && ProductCount[key].ToCountBy >= Discounts[key].amount)
                    {
                        Products.ForEach(p => { if (p.Name == key) p.Price = ProductCount[key].Actual * Discounts[key].percent * key.GetPriceByProductChar(Products); });
                        ProductCount[key] = (1, ProductCount[key].ToCountBy - Discounts[key].amount);
                    }
                }
            }
        }
